...
func (v *markdownRefetchCmd) flags(f *kit.FlagSet) {
	...
	f.StringVar(&v.repo, "repo", "open-index/open-markdown-refetch-v1", "dataset repo on HuggingFace (org/name)")
	f.IntVar(&v.workers, "workers", 0, "workers converting HTML to Markdown (0 = NumCPU)")
	...
	f.IntVar(&v.rate, "rate", 0, "request rate limit per host (req/s, 0 = unlimited)")
