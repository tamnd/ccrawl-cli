...
{"host":"facebook.com","harmonic_pos":1,"in_degree":0,"url_count":0,"status_2xx":0,...}
```

Facebook has more than zero inbound links. Zero is an answer, and this was the absence of one, wearing the same clothes.

Every count on a host record is now a pointer, so a number nothing measured is left out of the JSON and leaves the cell blank in a table or CSV rather than being filled in with a guess. A count that was taken and came out zero still prints as `0`, which is the whole point of separating the two:

```
$ ccrawl host top --limit 1 -o jsonl
{"host":"facebook.com","registered_domain":"facebook.com","harmonic_pos":1,"harmonic_val":0,"pagerank_pos":6,...}

$ ccrawl host enrich facebook.com --degrees -o jsonl
{"host":"facebook.com","in_degree":41297183,"out_degree":118,...}
```

The column is still there in `-o csv` and `-o table` and the cell is simply empty, so the shape of the output does not change with what a run happened to measure. `host top` also fills `registered_domain` now, which it had been leaving blank for no reason: the registered domain is the host with its labels dropped, so there was never anything to fetch.
...
Losing the connection mid-response is now exit 8 like any other transport failure. A status the server sent is still exit 1, so an exhausted 503 does not send anything into a retry loop against a host that is up and refusing.
